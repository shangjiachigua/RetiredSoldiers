...
public class Constance {
    ...
    public static String simpleTime(String time) {
        ...
        SimpleDateFormat formatOld = new SimpleDateFormat("HHmmss");
        SimpleDateFormat formatNew = new SimpleDateFormat("HH:mm:ss");
        Date date = null;
        try {
            date = formatOld.parse(time);
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    public static String randNumber(int digit) {
        String str = "";
        if (digit == 10) {
            str = Constance.simpleDateOne().substring(2, 8);
